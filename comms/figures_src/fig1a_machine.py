...
A(drum+0.55*(Ta-drum),"hoist rope",prefer=[(-0.7,0.7)])
...
A(bail,"dipper bail, yoke and rope socket (point $E$)",prefer=[(1,0.3)])
A(lug1,"bail pins in the dipper lugs",prefer=[(1,-0.4)])
A(arc_a[len(arc_a)//2],r"$\alpha_3$, angle between hoist rope and handle axis",prefer=[(-1,-0.3)])
A(E+0.5*h,"dipper",prefer=[(0.5,-1)])
A(np.array([14.4,2.0]),"bank",prefer=[(0,1)])
miss=C.solve()
